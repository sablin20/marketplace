...
public interface AnalyticsDao {
    ...
    List<ResponseDto> findNameStoreAndAmountCategoryForSales();

    //нужно получить пользователя(имя) и сколько он потратил на покупки какого-то бранда(бранд ты сам указываешь)
    List<ResponseDtoSumMoney> findNameClientAndSumCashByBrand(String brand);

    //нужно получить продавца(имя) который продал больше всего какого-то бранда
    List<ResponseDto> findNameStoreByMaxSalesByBrand(String brand);
    ...
    ResponseDto findNameClientAndCountIn3Category(Integer clientId);
    ...
    List<ResponseDto> findNameBrandAndAmountCategoryByPrice(BigInteger priceLimit);

    //показать все показать среднюю цену товара для категории (категорию передать надо)
    ResponseDtoAvgPrice findAvgPriceByCategory(String category);

    //показать категорию и самую большую цену для этой категории
    ResponseDtoMaxPrice findCategoryAndMaxPrice(String category);

    //показать самый дорогой товар для каждого бренда
    List<ResponseDtoMaxPrice> findProductByMaxPriceBrand();
    ...
    List<ResponseDto> findStoreAndAmountProductsOneBrand(String brand);

    //показать продавца и денег на сколько он продал товара определенного бренда (передать бранд)
    List<ResponseDtoSumMoney> findStoreAndSumCashByBrand(String brand);

    //показать покупателей и сколько они потратили денег на покупки у продавца (продавца надо передать)
    List<ResponseDtoSumMoney> findClientAndCashByStore(Integer storeId);
}